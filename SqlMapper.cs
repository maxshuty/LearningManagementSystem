...
namespace Dapper
{
    [AttributeUsage(AttributeTargets.Field | AttributeTargets.Property | AttributeTargets.Interface | AttributeTargets.Class, Inherited = false, AllowMultiple = false)]
    public class SqlNameAttribute : Attribute, SqlMapper.INamedMemberItem
    {
        private string name;
        ...
        protected virtual string GetName(MemberInfo mi)
        {
            return name;
        }

        string SqlMapper.INamedMemberItem.GetName(MemberInfo mi)
        {
            return GetName(mi);
        }
    }
    ...
    {
        public interface INamedMemberItem
        {
            string GetName(MemberInfo mi);
        }
                ...
                if ((deserializer = cinfo.Deserializer) == null || (otherDeserializers = cinfo.OtherDeserializers) == null)
                {
                    ...
                    Func<Type, int> nextSplit = type =>
                    {
                        ...
                        if (type != typeof(Object))
                        {
                            ...
                            foreach (var name in props.Select(p => p.SqlName).Concat(fields.Select(f => f.SqlName)))
                            {
...
#pragma warning restore 618
        ...
        static List<PropFieldInfo> GetSettableProps(Type t)
        {
            return t
                    .GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
                    .Select(p => new PropFieldInfo
                    {
                        Name = p.Name,
                        Setter = p.DeclaringType == t ? p.GetSetMethod(true) : p.DeclaringType.GetProperty(p.Name).GetSetMethod(true),
                        Type = p.PropertyType,
                        SqlName = ((INamedMemberItem)(p.GetCustomAttributes(typeof(INamedMemberItem), true).FirstOrDefault()) ?? new SqlNameAttribute(p.Name)).GetName(p)
                    })
                    .Where(info => info.Setter != null)
                    .ToList();
        }

        static List<PropFieldInfo> GetSettableFields(Type t)
        {
            return t
                    .GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
                    .Select(f => new PropFieldInfo
                    {
                        Name = f.Name,
                        Field = f,
                        Type = f.FieldType,
                        SqlName = ((INamedMemberItem)(f.GetCustomAttributes(typeof(INamedMemberItem), true).FirstOrDefault()) ?? new SqlNameAttribute(f.Name)).GetName(f)
                    })
                    .ToList();
        }
...
IDataReader reader, int startBound = 0, int length = -1, bool returnNullIfFirstMissing = false
...
)
        {
            ...
            var setters = (
                            from n in names
                            select properties.FirstOrDefault(p => string.Equals(p.SqlName, n, StringComparison.Ordinal)) // property case sensitive first
                                  ?? properties.FirstOrDefault(p => string.Equals(p.SqlName, n, StringComparison.OrdinalIgnoreCase))
                                 ?? fields.FirstOrDefault(p => string.Equals(p.SqlName, n, StringComparison.Ordinal)) // field case sensitive third
                                ?? fields.FirstOrDefault(p => string.Equals(p.SqlName, n, StringComparison.OrdinalIgnoreCase)) // field case insensitive fourth
                          ).ToList();
